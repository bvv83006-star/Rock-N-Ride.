...
const DEFAULT_SETTINGS = {
  steerType: 'buttons', tiltSens: 50, steerSens: 70, autoAccel: false, invertSteer: false, vibration: true,
  gfxQuality: 'medium', shadows: true, bloom: true, motionBlur: false, drawDist: 70,
  volMaster: 100, volEngine: 80, volMusic: 60, volSfx: 100,
  units: 'kmh', minimap: true, damage: false, showFps: false, traffic: 'medium', camera: 'chase'
};
const loadSettings = () => {
...
function getStats(car) {
  const u = save.upgrades[car.id] || {};
  return {
    speed: car.speed + (u.speed || 0) * 4,
    accel: car.accel + (u.accel || 0) * 3,
    grip: car.grip + (u.grip || 0) * 0.05
  };
}

// ===== SCENE =====
const scene = new THREE.Scene();
scene.fog = new THREE.FogExp2(0xa8c8e0, 0.0009);
...
const hemi = new THREE.HemisphereLight(0x88bbee, 0x2a2a30, 0.85);
scene.add(hemi);
const sun = new THREE.DirectionalLight(0xffffff, 2.3);
sun.castShadow = settings.shadows;
const shadowSize = settings.gfxQuality === 'ultra' ? 2048 : settings.gfxQuality === 'high' ? 2048 : 1024;
...
// Sky
const skyUniforms = {
  top: { value: new THREE.Color(0x3a8ee8) },
  mid: { value: new THREE.Color(0x80b8e8) },
  bot: { value: new THREE.Color(0xc0e0ff) },
...
function applyTimeOfDay() {
  ...
  skyUniforms.top.value.setHex(k.top);
  skyUniforms.mid.value.setHex(k.mid);
  skyUniforms.bot.value.setHex(k.bot);
  ...
  const sx = Math.cos(sunT) * 400, sy = Math.sin(sunT) * 400;
  sun.position.set(sx, Math.max(-40, sy), 200);
  sun.target.position.set(0, 0, 0);
  moon.position.set(-sx, Math.max(30, -sy), -200);
...
function addStreet(x, z, w, d) {
  const m = new THREE.Mesh(new THREE.PlaneGeometry(w, d), MAT.asphalt);
...
function addBuilding(x, z, w, d, h, mat) {
  ...
  if (Math.random() < 0.4) {
    ...
    ant.position.set(x, h + 4.5, z);
    scene.add(ant);
    const beacon = new THREE.Mesh(new THREE.SphereGeometry(0.3, 8, 6), new THREE.MeshBasicMaterial({ color: 0xff2222 }));
    beacon.position.set(x, h + 8.5, z);
    scene.add(beacon);
  }
...
(function () {
  ...
  for (let i = -80; i <= 80; i += 40) {
    addStreet(cx + i, cz, 12, 200);
    addStreet(cx, cz + i, 200, 12);
    const l1 = new THREE.Mesh(new THREE.PlaneGeometry(0.3, 200), MAT.yellow);
    l1.rotation.x = -Math.PI / 2;
    l1.position.set(cx + i, 0.04, cz);
    scene.add(l1);
    const l2 = new THREE.Mesh(new THREE.PlaneGeometry(200, 0.3), MAT.yellow);
    l2.rotation.x = -Math.PI / 2;
    l2.position.set(cx, 0.04, cz + i);
    scene.add(l2);
  }
  const positions = [[-60,-60,20,20,90],[60,-60,22,22,110],[-60,60,18,18,70],[60,60,20,20,100],[-20,-60,14,14,60],[20,60,16,16,80],[-60,20,14,14,75],[60,-20,16,16,95]];
  for (const [dx,dz,w,d,h] of positions) addBuilding(cx+dx, cz+dz, w, d, h, MAT.glass);
})();

// Residential
(function () {
  const cx = -300, cz = 200;
  for (let i = -80; i <= 80; i += 40) { addStreet(cx+i, cz, 10, 200); addStreet(cx, cz+i, 200, 10); }
  for (let i = 0; i < 12; i++) addBuilding(cx+(Math.random()-0.5)*160, cz+(Math.random()-0.5)*160, 10, 10, 8 + Math.random()*6, MAT.resi);
  for (let i = 0; i < 30; i++) {
    const dx = (Math.random()-0.5)*180, dz = (Math.random()-0.5)*180;
    const t = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.6, 3, 6), MAT.trunk);
    t.position.set(cx+dx, 1.5, cz+dz); scene.add(t);
    const l = new THREE.Mesh(new THREE.IcosahedronGeometry(2.5, 0), MAT.leaf);
    l.position.set(cx+dx, 4.5, cz+dz); scene.add(l);
  }
})();
...
(function () {
  const cx = 300, cz = -200;
  for (let i = -80; i <= 80; i += 50) { addStreet(cx+i, cz, 14, 200); addStreet(cx, cz+i, 200, 14); }
  for (let i = 0; i < 8; i++) addBuilding(cx+(Math.random()-0.5)*150, cz+(Math.random()-0.5)*150, 25, 18, 12, MAT.industrial);
  const cc = [0xdd3322, 0x2c7cff, 0xffbf35, 0x38f7c0];
  for (let i = 0; i < 20; i++) {
    const dx = (Math.random()-0.5)*160, dz = (Math.random()-0.5)*160;
    const m = new THREE.MeshStandardMaterial({ color: cc[Math.floor(Math.random()*4)], roughness: 0.7 });
    for (let s = 0; s < 1 + Math.floor(Math.random()*3); s++) box(6, 2.6, 2.5, m, cx+dx, 1.3 + s*2.7, cz+dz);
  }
})();
...
(function () {
  const cx = -300, cz = 0;
  const sand = new THREE.Mesh(new THREE.PlaneGeometry(180, 100), MAT.sand);
  sand.rotation.x = -Math.PI/2; sand.position.set(cx, 0.05, cz-40); sand.receiveShadow = settings.shadows; scene.add(sand);
  const water = new THREE.Mesh(new THREE.PlaneGeometry(180, 120), MAT.water);
  water.rotation.x = -Math.PI/2; water.position.set(cx, 0.03, cz-140); scene.add(water);
  box(180, 0.25, 8, MAT.sidewalk, cx, 0.125, cz+20);
  for (let i = 0; i < 15; i++) {
    const dx = (Math.random()-0.5)*160, dz = Math.random()*40;
    const t = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.5, 6, 6), new THREE.MeshStandardMaterial({ color: 0x6a4a2a, roughness: 0.95 }));
    t.position.set(cx+dx, 3, cz+dz); scene.add(t);
    const l = new THREE.Mesh(new THREE.IcosahedronGeometry(2, 0), new THREE.MeshStandardMaterial({ color: 0x2a7a3a, roughness: 0.9 }));
    l.position.set(cx+dx, 7, cz+dz); scene.add(l);
  }
  const lh = new THREE.Mesh(new THREE.CylinderGeometry(3, 4, 25, 12), MAT.concrete);
  lh.position.set(cx-60, 12.5, cz+10); lh.castShadow = settings.shadows; scene.add(lh);
  const lht = new THREE.Mesh(new THREE.CylinderGeometry(5, 5, 4, 12), new THREE.MeshStandardMaterial({ color: 0xff3b30, roughness: 0.5 }));
  lht.position.set(cx-60, 27, cz+10); scene.add(lht);
})();

// Central Plaza
(function () {
  const p = new THREE.Mesh(new THREE.CylinderGeometry(60, 60, 0.3, 32), MAT.sidewalk);
  p.position.set(0, 0.15, 0); p.receiveShadow = settings.shadows; scene.add(p);
  const fb = new THREE.Mesh(new THREE.CylinderGeometry(8, 10, 2, 16), MAT.concrete);
  fb.position.set(0, 1, 0); scene.add(fb);
  const fw = new THREE.Mesh(new THREE.CylinderGeometry(7, 7, 0.5, 16), MAT.water);
  fw.position.set(0, 2.2, 0); scene.add(fw);
  const ft = new THREE.Mesh(new THREE.ConeGeometry(2, 6, 8), MAT.concrete);
  ft.position.set(0, 5.5, 0); scene.add(ft);
})();

// Airport
(function () {
  const cx = 300, cz = 0;
  const rw = new THREE.Mesh(new THREE.PlaneGeometry(340, 40), MAT.runway);
  rw.rotation.x = -Math.PI/2; rw.position.set(cx, 0.02, cz); rw.receiveShadow = settings.shadows; scene.add(rw);
  for (let i = -20; i <= 20; i++) {
    const d = new THREE.Mesh(new THREE.PlaneGeometry(6, 0.7), MAT.white);
    d.rotation.x = -Math.PI/2; d.position.set(cx+i*8, 0.04, cz); scene.add(d);
  }
  addBuilding(cx-20, cz+80, 100, 20, 14, MAT.concrete);
  box(8, 40, 8, MAT.concrete, cx+40, 20, cz+80);
  box(14, 7, 14, MAT.glass, cx+40, 43.5, cz+80);
  const pb = new THREE.Mesh(new THREE.CylinderGeometry(2, 2, 22, 12), MAT.concrete);
  pb.rotation.z = Math.PI/2; pb.position.set(cx-20, 4, cz+60); scene.add(pb);
  const wing = new THREE.Mesh(new THREE.BoxGeometry(4, 0.5, 20), MAT.concrete);
  wing.position.set(cx-20, 4, cz+60); scene.add(wing);
})();

// Mountain
(function () {
  const cx = -300, cz = -200;
  const g = new THREE.Mesh(new THREE.PlaneGeometry(200, 200), MAT.grass);
  g.rotation.x = -Math.PI/2; g.position.set(cx, 0.04, cz); g.receiveShadow = settings.shadows; scene.add(g);
  const m = new THREE.Mesh(new THREE.ConeGeometry(80, 90, 8), new THREE.MeshStandardMaterial({ color: 0x5a5a4a, roughness: 0.95 }));
  m.position.set(cx, 45, cz); m.castShadow = settings.shadows; scene.add(m);
  for (let a = 0; a < Math.PI*2; a += 0.1) {
    const seg = new THREE.Mesh(new THREE.PlaneGeometry(8, 6), MAT.asphalt);
    seg.rotation.x = -Math.PI/2; seg.rotation.z = -a;
    seg.position.set(cx + Math.cos(a)*90, 0.06, cz + Math.sin(a)*90);
    scene.add(seg);
  }
})();
...
// Highway
(function () {
  const cx = 0, cz = 200, ringR = 100, seg = 32;
  for (let i = 0; i < seg; i++) {
    const a0 = (i/seg)*Math.PI*2, a1 = ((i+1)/seg)*Math.PI*2;
    const mx = cx + Math.cos((a0+a1)/2)*ringR;
    const mz = cz + Math.sin((a0+a1)/2)*ringR;
    const p = new THREE.Mesh(new THREE.BoxGeometry(20, 1, 21), MAT.concrete);
    p.position.set(mx, 10, mz); p.rotation.y = -(a0+a1)/2;
    p.castShadow = settings.shadows; p.receiveShadow = settings.shadows; scene.add(p);
    const r = new THREE.Mesh(new THREE.BoxGeometry(20, 1, 0.3), MAT.neon);
    r.position.set(mx, 11, mz); r.rotation.y = -(a0+a1)/2;
    scene.add(r);
  }
  for (let i = 0; i < 8; i++) {
    const a = (i/8)*Math.PI*2;
    const pil = new THREE.Mesh(new THREE.CylinderGeometry(1.4, 1.8, 10, 8), MAT.concrete);
    pil.position.set(cx + Math.cos(a)*ringR, 5, cz + Math.sin(a)*ringR);
    pil.castShadow = settings.shadows; scene.add(pil);
  }
})();

// Bowl
const BOWL = { x: 300, z: 200, r: 50, h: 32 };
(function () {
  ...
  const rim = new THREE.Mesh(new THREE.TorusGeometry(BOWL.r*0.85, 0.4, 8, 48), MAT.neon);
  rim.rotation.x = Math.PI/2; rim.position.set(BOWL.x, BOWL.h, BOWL.z); scene.add(rim);
  const pad = new THREE.Mesh(new THREE.CircleGeometry(6, 32), new THREE.MeshStandardMaterial({ color: 0xffd400, emissive: 0xffd400, emissiveIntensity: 1.2, transparent: true, opacity: 0.7 }));
  pad.rotation.x = -Math.PI/2; pad.position.set(BOWL.x, 0.06, BOWL.z); scene.add(pad);
  const ar = new THREE.Mesh(new THREE.BoxGeometry(16, 0.6, 36), MAT.ramp);
  ar.position.set(BOWL.x, 6, BOWL.z + BOWL.r*0.7);
  ar.rotation.x = -Math.atan2(12, 36);
  scene.add(ar);
})();

const RAMPS = [
  { x: 0, z: -100, rot: Math.PI/2, w: 12, d: 32, h: 5 },
  { x: 0, z: 100, rot: -Math.PI/2, w: 12, d: 32, h: 5 },
  { x: -100, z: 0, rot: 0, w: 12, d: 32, h: 5 },
  { x: 100, z: 0, rot: Math.PI, w: 12, d: 32, h: 5 },
  { x: 200, z: -300, rot: Math.PI/2, w: 12, d: 32, h: 5 },
  { x: -200, z: 300, rot: 0, w: 12, d: 32, h: 5 }
];
...
async function preloadCars(onProgress) {
  let done = 0;
  const total = CAR_CLASSES.length;
...
function makeFallbackCar(colorHex) {
  ...
  body.position.y = 0.5; body.castShadow = settings.shadows;
  g.add(body);
...
function buildCar(classId, colorHex) {
  ...
  if (!template) {
    ...
    outer.add(fb);
    outer.userData.bodyMaterial = fb.children[0].material;
    return outer;
  }

  let model;
  try { model = template.clone(true); }
  catch(e) { const fb = makeFallbackCar(colorHex); outer.add(fb); return outer; }
...
// Apply steer type from settings
if (settings.steerType === 'wheel') {
  if (btnLeft) btnLeft.style.display = 'none';
  if (btnRight) btnRight.style.display = 'none';
  if (steerWheel) steerWheel.style.display = 'block';
} else {
...
function addSkid(x, z, rotY) {
  ...
  s.mesh.visible = true; s.mesh.material.opacity = 0.6;
  s.life = 4;
...
// Roblox meme chip-tune
let musicTimer = null;
const MELODY = [
  [523.25, 1], [659.25, 1], [783.99, 1], [659.25, 1], [1046.50, 4],
  [587.33, 1], [698.46, 1], [880.00, 1], [698.46, 1], [1174.66, 4],
  [493.88, 1], [587.33, 1], [698.46, 1], [587.33, 1], [880.00, 3], [783.99, 1], [659.25, 2]
];
const BASS = [130.81, 130.81, 146.83, 146.83, 164.81, 164.81, 130.81, 130.81];
let mIdx = 0, bIdx = 0, mStep = 0, bStep = 0;
function startMusic() {
...
function updateHud(dt) {
  ...
  if (P.drifting && P.driftScore > 0) {
    ...
    driftPop.classList.remove('show');
  }
}
...
function updatePlayer(dt) {
  ...
  // Wheel
  if (settings.steerType === 'wheel' && wheelDragging) {
    steer = wheelAngle;
  }
  // Invert
  ...
  if (P.drifting && speedAbs > 15 && !P.airborne) {
    ...
    addSkid(lX, lZ, P.heading);
    addSkid(rX, rZ, P.heading);
    for (let k = 0; k < 3; k++) {
