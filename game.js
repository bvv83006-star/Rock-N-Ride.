...
Object.assign(sun.shadow.camera, { left:-400, right:400, top:400, bottom:-400, near:1, far:1500 });
...
function applyTimeOfDay() {
  ...
  sun.position.set(sx, Math.max(-40, sy), 200); sun.target.position.set(0,0,0);
...
(function() {
  const cx = 350, cz = -350;
  for (let i = -140; i <= 140; i += 70) { addStreet(cx+i, cz, 10, 300); addStreet(cx, cz+i, 300, 10); }
  const towers = [[-100,-100,32,32,180],[0,-120,38,38,220],[100,-100,30,30,160],[-120,0,34,34,200],[0,0,42,42,240],[120,0,36,36,190],[-100,100,28,28,150],[0,120,40,40,210],[100,100,32,32,175],[-60,-60,22,22,120],[60,-60,24,24,140],[-60,60,20,20,110],[60,60,26,26,130]];
...
(function() {
  ...
  for (let i = 0; i < 80; i++) {
    const dx = (Math.random()-0.5)*380, dz = (Math.random()-0.5)*380;
...
(function() {
  ...
  for (let i = 0; i < 40; i++) {
    const dx = (Math.random()-0.5)*380, dz = 20 + Math.random()*60;
...
(function() {
  const cx = 350, cz = 350;
  for (let i = -100; i <= 100; i += 50) { addStreet(cx+i, cz-150, 12, 300); addStreet(cx-150, cz+i, 300, 12); }
  for (let i = 0; i < 20; i++) {
    const dx = (Math.random()-0.5)*300, dz = (Math.random()-0.5)*300;
...
// Central Plaza
(function() {
  const p = new THREE.Mesh(new THREE.CylinderGeometry(90,90,0.4,48), MAT.sidewalk);
  p.position.set(0,0.2,0); p.receiveShadow = settings.shadows; scene.add(p);
...
(function() {
  ...
  for (let i = 0; i < seg; i++) {
    ...
    if (i % 4 === 0) {
      ...
      pil.position.set(mx,7,mz); pil.castShadow = settings.shadows; scene.add(pil);
    }
  }
})();
...
function attachBodyKits(carGroup, classId) {
  ...
  if (bk.spoiler !== 'none') {
    ...
    if (bk.spoiler === 'sport') {
      ...
      for (const sx of [-1,1]) {
        const leg = new THREE.Mesh(new THREE.BoxGeometry(0.1,0.25,0.15),spoilerMat);
        leg.position.set(sx*0.6,1.4,2.1); g.add(leg);
      }
      ...
      for (const sx of [-1,1]) {
        const leg = new THREE.Mesh(new THREE.BoxGeometry(0.12,0.5,0.2),spoilerMat);
        leg.position.set(sx*0.8,1.5,2.15); g.add(leg);
      }
      ...
      for (const sx of [-1,1]) {
        const leg = new THREE.Mesh(new THREE.BoxGeometry(0.15,0.7,0.15),spoilerMat);
        leg.position.set(sx*0.75,1.6,2.1); g.add(leg);
      }
    }
